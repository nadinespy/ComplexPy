...
`emergence_complexity_measures_comparison/emergence_complexity_measures_comparison.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `emergence_complexity_measures_comparison.py` in directory called
`emergence_complexity_measures_comparison`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import emergence_complexity_measures_comparison as sb` in an
interactive Python session, the classes and functions defined inside of the
`emergence_complexity_measures_comparison.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the emergence_complexity_measures_comparison module, the main classes defined at the bottom of
...
Either way, you can create a `emergence_complexity_measures_comparison/data` folder in which you can
...
(typically, something like our `emergence_complexity_measures_comparison/tests/test_emergence_complexity_measures_comparison.py`). Within each
...
have one such test in `emergence_complexity_measures_comparison/tests/test_emergence_complexity_measures_comparison.py` called
...
To document `emergence_complexity_measures_comparison` we use the [sphinx documentation
...
A `emergence_complexity_measures_comparison/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/emergence_complexity_measures_comparison) for the software. This
...
`emergence_complexity_measures_comparison` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `emergence_complexity_measures_comparison`, we use the
...
[here](https://github.com/uwescience/emergence_complexity_measures_comparison/blob/master/scripts/Figure1.ipynb).
...
### Using `emergence_complexity_measures_comparison` as a template
...
button on the front page of the "emergence_complexity_measures_comparison" repository.
...
`emergence_complexity_measures_comparison/emergence_complexity_measures_comparison.py` to be called `smallish/smallish.py`

	git mv emergence_complexity_measures_comparison smallish
	git mv smallish/emergence_complexity_measures_comparison.py smallish/smallish.py
	git mv smallish/tests/test_emergence_complexity_measures_comparison.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `emergence_complexity_measures_comparison` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `emergence_complexity_measures_comparison` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `emergence_complexity_measures_comparison` in them. Type
the following to see where all these files are:

	git grep emergence_complexity_measures_comparison

You can replace `emergence_complexity_measures_comparison` for `smallish` quickly with:

	git grep -l 'emergence_complexity_measures_comparison' | xargs sed -i 's/emergence_complexity_measures_comparison/smallish/g'
...
Edit all the mentions of `emergence_complexity_measures_comparison` in the `emergence_complexity_measures_comparison/__init__.py` file, and in
the `emergence_complexity_measures_comparison/version.py` file as well.
